...
class MLAHead(nn.Module):
    ...
    def forward(self, mla_p2, mla_p3, mla_p4, mla_p5, scale_factor):
        ...
        head2 = F.interpolate(
            self.head2(mla_p2), scale_factor=scale_factor, mode="trilinear", align_corners=True
        )
        head3 = F.interpolate(
            self.head3(mla_p3), scale_factor=scale_factor, mode="trilinear", align_corners=True
        )
        head4 = F.interpolate(
            self.head4(mla_p4), scale_factor=scale_factor, mode="trilinear", align_corners=True
        )
        head5 = F.interpolate(
            self.head5(mla_p5), scale_factor=scale_factor, mode="trilinear", align_corners=True
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cls = nn.Sequential(
            nn.Conv3d(4 * mlahead_channels + 1, mlahead_channels, 3, padding=1, bias=False),
            nn.InstanceNorm3d(mlahead_channels),
            nn.ReLU(),
...
class VIT_MLAHead_h(nn.Module):
    ...
    def forward(self, inputs, scale_factor1, scale_factor2):
        x = checkpoint(self.mlahead, inputs[0], inputs[1], inputs[2], inputs[3], scale_factor1)
        x = torch.cat([x, inputs[-1]], dim=1)
        x = checkpoint(self.cls, x)
